...
class PlaywrightMCPRunner:
    ...
    def run_tests(self, test_type="all", project=None, headed=False, debug=False, workers=None):
        ...
        cmd.extend(["--reporter=html,json,junit", f"--output-dir={self.reports_dir}/test-results"])
    ...
    def generate_summary_report(self, exit_code, duration, test_type):
        ...
        if test_results.get("stats"):
            ...
            logger.info(
                f"   Tests: {stats.get('expected', 0)} passed, {stats.get('failed', 0)} failed"
            )
...
def main():
    ...
    parser.add_argument(
        "--type",
        choices=["all", "e2e", "integration", "performance", "visual"],
        default="all",
