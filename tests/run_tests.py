...
class MELDTestRunner:
    ...
    async def start_application(self):
        ...
        try:
            ...
            self.app_process = subprocess.Popen(
                [sys.executable, "-m", "meld_visualizer"], cwd=self.project_root
            )
    ...
    async def test_csv_file_upload(self):
        ...
        self.test_dir / "playwright" / "fixtures" / "test_data" / "sample_meld_data.csv"
    ...
    async def generate_final_report(self):
        ...
        # Save JSON report
        report_file = (
            self.reports_dir / f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--integration", action="store_true", help="Run Playwright integration tests"
    )
    parser.add_argument(
        "--performance", action="store_true", help="Run Playwright performance tests"
    )
    parser.add_argument(
        "--visual", action="store_true", help="Run Playwright visual regression tests"
    )

    # Application management
    parser.add_argument(
        "--start-app", action="store_true", help="Automatically start the application before tests"
    )
    parser.add_argument(
        "--app-url", default="http://localhost:8050", help="URL of the running application"
    )
