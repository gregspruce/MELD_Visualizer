...
from ..config import APP_CONFIG, PLOTLY_TEMPLATE, THEMES, get_responsive_plot_style
from ..constants import (
    CONFIG_ALERT_DURATION_MS,
    DEFAULT_VIEWPORT_HEIGHT,
    DEFAULT_VIEWPORT_WIDTH,
    DEFAULT_Z_STRETCH_FACTOR,
    UI_NUMERIC_INPUT_STEP,
...
def create_responsive_graph(graph_id, plot_type="scatter_3d", **kwargs):
    ...
    # Create empty figure locally to avoid circular imports
    import plotly.graph_objects as go

    from ..config import PLOTLY_TEMPLATE
    from ..constants import DEFAULT_FONT_SIZE

    def create_empty_figure_local(message="Upload a file and configure options."):
        """Create a blank Plotly figure with a text message (local version to avoid circular imports)."""
        fig = go.Figure()
        fig.update_layout(
    ...
    return dcc.Graph(
        ...
        figure=kwargs.pop("figure", create_empty_figure_local()),
        **kwargs,
    )
    ...
    return html.Div(
        [
            dcc.Store(
                ...
                data={"width": DEFAULT_VIEWPORT_WIDTH, "height": DEFAULT_VIEWPORT_HEIGHT},
            ),
            dcc.Interval(
                id="viewport-check-interval",
                interval=VIEWPORT_CHECK_INTERVAL_MS,
                n_intervals=0,
                disabled=True,
            ),
            html.Script(
...
def add_enhanced_ui_scripts():
    ...
    return html.Div(
        [
            html.Link(
                rel="stylesheet", href="/static/css/enhanced-desktop-ui.css", id="enhanced-ui-css"
            ),
            html.Script(src="/static/js/enhanced-ui.js", id="enhanced-ui-js"),
            html.Link(
                rel="stylesheet",
...
def build_header():
    ...
    return dbc.Row(
        [
            html.Div(
                id="output-filename",
                className="text-primary text-center fs-3 mb-2",
                children="Please upload a CSV file to begin.",
            ),
            dbc.Alert(
                id="config-warning-alert", color="warning", is_open=False, className="w-75 mx-auto"
            ),
...
def build_2d_plotter():
    ...
    return [
        dbc.Row(
            [
                html.H4("2D Scatter Plot", className="text-center"),
                dbc.Col(create_filter_controls("time-2d", "Time"), width=12),
            ],
            className="mb-2",
        ),
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H6("Y-Axis"),
                        dcc.RadioItems(id="radio-2d-y", options=[], value=None, inline=True),
                    ],
                    width=6,
                ),
                dbc.Col(
                    [
                        html.H6("Color Scale"),
                        dcc.RadioItems(id="radio-2d-color", options=[], value=None, inline=True),
                    ],
                    width=6,
                ),
            ],
            ...
            [
                dbc.Col(
                    [
                        html.H6("X-Axis"),
                        dcc.Dropdown(
                            id="custom-dropdown-x", options=[], value=None, clearable=False
                        ),
                    ],
                    width=3,
                ),
                dbc.Col(
                    [
                        html.H6("Y-Axis"),
                        dcc.Dropdown(
                            id="custom-dropdown-y", options=[], value=None, clearable=False
                        ),
                    ],
                    width=3,
                ),
                dbc.Col(
                    [
                        html.H6("Z-Axis"),
                        dcc.Dropdown(
                            id="custom-dropdown-z", options=[], value=None, clearable=False
                        ),
                    ],
                    width=3,
                ),
                dbc.Col(
                    [
                        html.H6("Color"),
                        dcc.Dropdown(
                            id="custom-dropdown-color", options=[], value=None, clearable=False
                        ),
                    ],
                    width=3,
                ),
            ...
            [
                dbc.Col(
                    [
                        html.H6("Filter By"),
                        dcc.Dropdown(
                            id="custom-dropdown-filter", options=[], value=None, clearable=False
                        ),
                    ],
                    width=3,
                ),
        ...
        children=[
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.H4("Application Appearance"),
                            html.Hr(),
                            dbc.Label("Application Theme"),
                            dcc.Dropdown(
                                id="config-theme-dropdown",
                                options=theme_options,
                                value=APP_CONFIG["default_theme"],
                            ),
                            html.Br(),
                            dbc.Label("Plot Background Template"),
                            dcc.Dropdown(
                                id="config-template-dropdown",
                                options=template_options,
                                value=PLOTLY_TEMPLATE,
                            ),
                            html.P(
                                "Note: If you select a dark theme, this should be 'plotly_dark'.",
                    ...
                    dbc.Col(
                        [
                            html.H4("Default Graph Options"),
                            html.Hr(),
                            dbc.Label("Graph 1 Options (Color Dropdown)"),
                            dcc.Dropdown(
                                id="config-graph-1-dropdown",
                                options=[],
                                value=APP_CONFIG["graph_1_options"],
                                multi=True,
                            ),
                            html.Br(),
                            dbc.Label("Graph 2 Options (Color Dropdown)"),
                            dcc.Dropdown(
                                id="config-graph-2-dropdown",
                                options=[],
                                value=APP_CONFIG["graph_2_options"],
                                multi=True,
                            ),
                            html.Br(),
                            dbc.Label("2D Plot Y-Axis Options"),
                            dcc.Dropdown(
                                id="config-2d-y-dropdown",
                                options=[],
                                value=APP_CONFIG["plot_2d_y_options"],
                                multi=True,
                            ),
                            html.Br(),
                            dbc.Label("2D Plot Color Options"),
                            dcc.Dropdown(
                                id="config-2d-color-dropdown",
                                options=[],
                                value=APP_CONFIG["plot_2d_color_options"],
                                multi=True,
                            ),
                        ],
                        md=6,
            ...
            dbc.Row(
                dbc.Col(
                    [
                        ...
                        dbc.Alert(
                            id="save-config-alert",
                            is_open=False,
                            duration=CONFIG_ALERT_DURATION_MS,
                            color="success",
                            className="mt-3",
                        ),
                        html.P(
                            "📌 Theme changes apply instantly! Graph option changes apply immediately after saving.",
        ...
        children=[
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        dbc.Button(
                            "Generate Toolpath Plot",
                            id="generate-line-plot-button",
                            color="primary",
                            className="mb-3",
                        ),
                        width="auto",
            ...
            dcc.Loading(
                id="loading-line-plot",
                type="circle",
                children=create_responsive_graph("line-plot-3d", "toolpath_3d"),
            ),
        ],
    )
    ...
    return html.Div(
        ...
        children=[
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [html.H6("Color by"), dcc.Dropdown(id="mesh-plot-color-dropdown")], width=4
                    ),
            ...
            dcc.Loading(
                id="loading-mesh-plot",
                type="circle",
                children=create_responsive_graph("mesh-plot-3d", "volume_mesh"),
            ),
        ],
    )
...
def build_gcode_tab():
    ...
    return html.Div(
        ...
        children=[
            ...
            dbc.Alert(
                id="gcode-filename-alert", color="info", is_open=False, className="w-75 mx-auto"
            ),
            dcc.Upload(
                id="upload-gcode",
                    ...
                    dbc.Col(
                        dbc.InputGroup(
                            [
                                dbc.InputGroupText("Z-Axis Stretch Factor"),
                                dbc.Input(
                                    id="gcode-z-stretch-input",
                                    type="number",
                                    value=DEFAULT_Z_STRETCH_FACTOR,
                                    min=0.1,
                                    step=UI_NUMERIC_INPUT_STEP,
                                ),
                            ],
...
def build_app_body_with_tabs():
    ...
    tabs_config = [
        ...
        {"id": "3d-toolpath-plot", "label": "3D Toolpath Plot", "content": build_line_plot_tab()},
        {"id": "3d-volume-mesh", "label": "3D Volume Mesh", "content": build_mesh_plot_tab()},
        {
            "id": "gcode-visualization",
            "label": "G-code Visualization",
            "content": build_gcode_tab(),
        },
        {"id": "settings", "label": "Settings", "content": build_config_tab()},
    ]

    return dbc.Tabs(
        [
            dbc.Tab(label=tab["label"], tab_id=tab["id"], children=tab["content"])
