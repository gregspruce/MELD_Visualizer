...
class MeshGenerator:
    ...
    def generate_cross_section(
        ...
        if half_n < 2:
            half_n = 2
            logger.warning(
                f"Adjusting points_per_section from {self.points_per_section} to 4 to avoid degenerate geometry"
            )
...
class VolumePlotter:
    ...
    def prepare_data(
        self, df: pd.DataFrame, min_feed_velocity: float = 0.1, min_path_velocity: float = 0.1
    ) -> pd.DataFrame:
        """
    ...
    def generate_plot_data(
        self, df: pd.DataFrame, color_column: str, lod: str = "high"
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def set_calibration(self, correction_factor: float = 1.0, area_offset: float = 0.0) -> None:
        """
    ...
    def export_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mesh_generator": {"points_per_section": self.mesh_generator.points_per_section},
        }


# Convenience functions for backward compatibility
def generate_volume_mesh_from_df(
    df: pd.DataFrame, color_column: str, lod: str = "high"
) -> Optional[Dict[str, Any]]:
    """
