...
@handle_errors(
    error_type=MELDVisualizerError,
    error_code=ErrorCode.CONFIGURATION_ERROR,
    user_message="Theme configuration error. Using default theme.",
)
def _resolve_external_stylesheets():
    ...
    with ErrorContext("Theme resolution", logger, error_code=ErrorCode.CONFIGURATION_ERROR):
        try:
            ...
            if isinstance(APP_CONFIG, dict):
                key = (
                    APP_CONFIG.get("default_theme")
                    or APP_CONFIG.get("theme")
                    or APP_CONFIG.get("bootstrap_theme")
                )
            if key and isinstance(THEMES, dict) and key in THEMES:
                ...
                if isinstance(val, str) and (
                    ...
                    theme = val or theme
                ErrorLogger.log_info(logger, f"Loaded theme: {key}", {"theme": theme})
            elif key and hasattr(dbc.themes, key):
                theme = getattr(dbc.themes, key)
                ErrorLogger.log_info(logger, f"Loaded dbc theme: {key}")
        except ImportError as e:
            ErrorLogger.log_warning(logger, f"Config module not found, using default theme: {e}")
        except KeyError as e:
            ErrorLogger.log_warning(
                logger, f"Theme configuration key not found, using default: {e}"
            )
...
def _build_layout(app):
    ...
    return html.Div(
        [html.H1(APP_TITLE), html.P("Fallback layout: layout.py not found or failed to load.")]
    )
