...
def register_config_callbacks(app=None):
    ...
    def save_config_and_advise_restart(
        n_clicks, theme, template, g1_opts, g2_opts, y_2d_opts, color_2d_opts
    ):
    ...
    def update_main_graph_radios(column_ranges, reload_trigger):
        ...
        try:
            ...
            valid_opts_1 = [
                opt for opt in current_config.get("graph_1_options", []) if opt in df_cols
            ]
            valid_opts_2 = [
                opt for opt in current_config.get("graph_2_options", []) if opt in df_cols
            ]
    ...
    def update_2d_plot_radios(column_ranges, reload_trigger):
        ...
        try:
            ...
            valid_y_opts = [
                opt for opt in current_config.get("plot_2d_y_options", []) if opt in df_cols
            ]
            valid_color_opts = [
                opt for opt in current_config.get("plot_2d_color_options", []) if opt in df_cols
            ]
    ...
    def update_custom_and_mesh_plot_controls(layout_config):
        ...
        default_x = (
            DEFAULT_X_AXIS
            if DEFAULT_X_AXIS in axis_options
            else (axis_options[0] if axis_options else None)
        )
        default_y = (
            DEFAULT_Y_AXIS
            if DEFAULT_Y_AXIS in axis_options
            else (axis_options[1] if len(axis_options) > 1 else None)
        )
        default_z = (
            DEFAULT_Z_AXIS
            if DEFAULT_Z_AXIS in axis_options
            else (axis_options[2] if len(axis_options) > 2 else None)
        )
        default_color = (
            DEFAULT_COLOR_COLUMN
            if DEFAULT_COLOR_COLUMN in axis_options
            else (axis_options[3] if len(axis_options) > 3 else default_x)
        )
        default_filter = (
            DEFAULT_FILTER_COLUMN if DEFAULT_FILTER_COLUMN in axis_options else default_x
        )
